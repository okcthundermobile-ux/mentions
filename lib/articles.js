...
export async function gatherArticles(query, range, apiKeys = {}) {
  const [cse, newsapi, apify] = await Promise.all([
    searchGoogleCSE(query, range, apiKeys),
    searchNewsApi(query, range, apiKeys),
    gatherApify(query, range, apiKeys),
  ]);
...
async function searchGoogleCSE(query, range, apiKeys = {}) {
  const key = apiKeys.googleCseKey || process.env.GOOGLE_CSE_KEY;
  const cx = apiKeys.googleCseId || process.env.GOOGLE_CSE_ID;
  if (!key || !cx) return [];
  try {
...
async function searchNewsApi(query, range, apiKeys = {}) {
  const key = apiKeys.newsApiKey || process.env.NEWS_API_KEY;
  if (!key) return [];
  try {
    const params = new URLSearchParams({
