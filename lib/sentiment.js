...
async function callGemini(prompt, apiKeys = {}) {
  ...
  if (!key) throw new Error('Gemini API key is not set. Add GEMINI_API_KEY to .env.local, or paste it on the Connect APIs page.');
...
async function scoreBatch(query, texts, apiKeys) {
...
export async function buildReport(query, posts, apiKeys = {}) {
  ...
  for (let start = 0; start < posts.length; start += 20) {
    ...
    try {
      const scores = await scoreBatch(query, batch.map((p) => p.text), apiKeys);
